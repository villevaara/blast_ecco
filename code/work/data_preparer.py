...
class DataPreparer:
    ...
    def __init__(self, data_location, output_folder, threads, language, split_size, gpu, logger):
    ...
    def make_db(self):
        subprocess_params = ("makeblastdb -in {} -dbtype prot -title TextDB -parse_seqids -hash_index -out {}".format(self.output_folder + "/db/database.fsa", self.output_folder + "/db/textdb").split(" "))
        if self.gpu is not None:
            if self.gpu == "T":
                subprocess_params.append("-sort_volumes")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpu", help="Prepare the DB for GPU?")
    args = parser.parse_args()

    logger = get_logger()

    dp = DataPreparer(args.data_location, args.output_folder, args.threads, args.language, args.split_size, args.gpu, logger)
    dp.prepare_data()
