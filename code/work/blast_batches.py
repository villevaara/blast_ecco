...
def get_folder_size(folder):
    return int(subprocess.check_output(
        ["du", "-s", folder]).split()[0].decode("utf-8"))
...
def copy_output_folder_to_local(
        output_folder, local_folder, wait=True, wait_time=5):
        ...
        if os.path.exists(local_folder):
            ...
            while True:
                ...
                if current_size < original_size:
                    print("{} < {}, waiting {} seconds.".format(current_size,
                          original_size, wait_time))
            ...
            try:
                ...
                time.sleep(wait_time)
                copy_output_folder_to_local(
                    output_folder, local_folder, wait, wait_time)
    else:
        copytree(output_folder, local_folder)
...
def copy_local_data_back(output_folder, batch_folder, iter):
    ...
    copyfile(output_folder + "/batches/iter_{}.tar.gz".format(iter),
             batch_folder + "/iter_{}.tar.gz".format(iter))
...
def run_taito(args):
    ...
    args.local_folder = (
        os.environ.get("TMPDIR") + "/" + args.output_folder.split("/")[-1])
    run_normal(args)
